...
function fmtHourShort(d){const h=d.getHours(),suffix=h<12?"a":"p";let hh=h%12;if(hh===0)hh=12;return `${hh}${suffix}`}
// Horizontally scrollable bar chart: one bar per hour, height/color show
// the score, day boundaries get a short weekday label instead of a
// separate divider row. Tap a bar to see that hour's full detail below.
let lastWindows=[];
function renderWindowsChart(windows){
  lastWindows=windows;
  let lastDay=null;
  return windows.map((w,i)=>{
    const dayKey=w.start.toDateString();
    const isNewDay=dayKey!==lastDay;
    lastDay=dayKey;
    const label=(isNewDay?new Intl.DateTimeFormat("en-US",{weekday:"short"}).format(w.start)+" ":"")+fmtHourShort(w.start);
    const heightPct=Math.max(4,w.score);
    return `<button type="button" class="chart-bar-wrap${isNewDay?" chart-new-day":""}" data-idx="${i}" aria-label="${escapeHtml(fmtTime(w.start))}, score ${w.score}"><span class="chart-score">${w.score}</span><span class="chart-bar-track"><span class="chart-bar ${w.verdict.toLowerCase()}" style="height:${heightPct}%"></span></span><span class="chart-time">${escapeHtml(label)}</span></button>`;
  }).join("");
}
function showWindowDetail(idx){
  const w=lastWindows[idx];
  const el=$("windowDetail");
  if(!w||!el)return;
  el.innerHTML=`<strong>${escapeHtml(fmtTime(w.start))}</strong> — <span class="badge ${w.verdict.toLowerCase()}">${w.score}</span> ${escapeHtml(w.verdict)}<br>${w.temp??"—"}°F • Wind ${w.wind} mph • Rain ${w.rain}%<br>${escapeHtml(w.tide)}<br>${escapeHtml(w.reasons.slice(0,3).join(" • "))}`;
}
